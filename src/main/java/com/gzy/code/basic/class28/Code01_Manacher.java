...
public class Code01_Manacher {
   ...
   *      2. 中线点C, 也就是回文半径中心的下标索引
   *      3  准备一个回文半径数组，跟字符串长度一样长 pArr[]
   *          pArr[i] = R  ->  i 代表字符对应的索引， R代表以 chars[i] 为回文子串的中心，半径长度
   *      4. 遍历的步骤：
   *            index = i
   *          4.1 先获取 i 对应的回文半径，第一种情况：
   *              4.1.1 前一个面的回文半径一直没有更新， 也就是 pArr[C] + C = R > i，
   *                    代表 i 还在之前的回文半径内,
   *                    pArr[i] = (i 关于 C 对称点 i'的 pArr[i'] = pArr[2*C - i])
   ...
   *                    pArr[C] + C ==  i
   *                    虽然有对称点，但是对称点对应的回文半径越界了(i'的左边界 = i' - pArr[i'] 小于 C - R)，对当前的i没有参考意义，所以也赋予一个默认值1
   *          4.2 从 i -> R, 遍历所有的索引，看是否可以更新pArr[i],这个过程其实就是加速的过程
   *              i' 是 i 关于C的对称点
   *              4.2.1  i' 的左边界在 C的左边界内
   *                这种情况直接跳过， pArr[i] == pArr[i'], 由于是在大的以C为中心的回文串中，不需要比较
   *              4.2.2  i' 的左边界==C的左边界
   *                这种情况也不要比较， 因为如果 chars[i - pArr[i] - 1] == (chars[i + pArr[i] +1] = chars[R + 1])
   *                                  那么根据对称性， chars[i' - pArr[i'] - 1] ==  chars[R + 1]，那么就会推翻 R是目前C的最大回文半径，
   *                                  R要+1， 显然不成立
   *              4.2.3  i' 的左边界小于C的左边界
