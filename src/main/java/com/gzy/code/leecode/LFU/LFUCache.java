...
class LFUCache {
    ...
    /**
     * 分析： 介于之前实现过LRU，在LRU基础上再加个计数器记录使用的次数
     * 要求： 必须以O(1)的平均时间复杂度
     *
     *
     * @param capacity
    ...
    public void put(int key, int value) {
        ...
        if(node == null){
            ...
            if(map.size() == capacity){
                Node poll = heap.first();
                heap.remove(poll);
                map.remove(poll.key);
//                --size;
            }

            node = new Node(key, value, ++this.time);
            map.put(key, node);
            heap.add(node);
//            ++size;
        }else {
